...
def write_asc(path, vertices):
    ...
    pc = trimesh.points.PointCloud(vertices[:,:3], colors=vertices[:,3:])
    pc.export(path)
...
# def batch_orth_proj_inv(X_trans, camera):
#     ''' remove orthgraphic projection
#         X:  3d vertices, [bz, n_point, 3]
#         camera: scale and translation, [bz, 3], [scale, tx, ty]
#     '''
#     camera = camera.clone().view(-1, 1, 3)
#     Xn = X_trans/camera[:, :, 0:1]

#     X_xy = Xn[:, :, :2] - camera[:, :, 1:]
#     X = torch.cat([X_xy, Xn[:,:,2:]], 2)
#     return X
...
def batch_orth_proj_inv(X_trans, camera):
    ...
    # X = torch.cat([X_xy, Xn[:,:,2:]], 2)
    # return X
    # print(X_trans.device, camera.device)
    # exit()
    # import ipdb; ipdb.set_trace()
    camera_n = camera.view(-1, 1, 3)
    Xn = X_trans/camera_n[:, :, 0:1]

    X_xy = Xn[:, :, :2] - camera_n[:, :, 1:]
    X = torch.cat([X_xy, Xn[:,:,2:]], 2)
    return X
...
def load_obj(obj_filename):
    ...
    for line in lines:
        ...
        if line.startswith("v "):  # Line is a vertex.
            ...
            vert = [float(x) for x in tokens[1:4+3]]
            # if len(vert) != 3:
...
import pickle
def save_params(param_path, params, ind=0):
    out_data = {}
    for k, v in params.items():
        if torch.is_tensor(v):
            ...
            for pk, pv in v.items():
                out_data[pk] = pv[ind].detach().cpu().numpy()
        else:
            out_data[k] = v
...
def visualize_grid(visdict, savepath=None, size=224, dim=2, return_gird=True, print_key=True, vis_keys=None, report_metric=False, pic='w'):
    '''
    ...
    #         d = loss_fn_alex(visdict['nerf_image'][:, :, :256, :], visdict['image'][:, :, :256, :])
    #     elif pic = 'df':
    #         psnr_value = psnr(visdict['nerf_image'][:, :, 256:, :], visdict['image'][:, :, :256, :])
    #         ssim_value = ssim(visdict['nerf_image'][:, :, 256:, :], visdict['image'][:, :, :256, :])
    #         d = loss_fn_alex(visdict['nerf_image'][:, :, 256:, :], visdict['image'][:, :, :256, :])
...
def load_textured_mesh(mesh_file, device):
    from pytorch3d.io import load_obj
    albedo = imread(mesh_file.replace('.obj', '.png'))/255.
    albedo = torch.from_numpy(albedo.transpose(2,0,1)).to(device).float()[None,...]
    verts, faces, aux = load_obj(mesh_file)
    verts = verts[None,...].to(device).float()
    mesh = {
        'vertices': verts,
        'albedo': albedo
    }
